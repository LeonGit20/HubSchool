...
/**
 * Mostra il modale di consenso cookie e blocca lo scrolling del body.
 */
function showCookieConsent() {
    if (cookieConsentOverlay) {
        cookieConsentOverlay.classList.add('show');
        document.body.classList.add('no-scroll'); // Blocca lo scrolling del body
    }
}

/**
 * Nasconde il modale di consenso cookie e riabilita lo scrolling del body.
 */
function hideCookieConsent() {
    if (cookieConsentOverlay) {
        cookieConsentOverlay.classList.remove('show');
        document.body.classList.remove('no-scroll'); // Riabilita lo scrolling del body
    }
}
...
function handleAcceptCookies() {
    ...
    hideCookieConsent();
    // Inizializza le funzionalità che dipendono dai cookie qui, inclusi i preferiti
    loadFavoritesFromCookie();
    if (!window.addProductsInitialized) { // Flag per evitare doppie inizializzazioni
        initAddProducts();
        window.addProductsInitialized = true;
    }
    // Ricarica i prodotti per assicurarsi che i pulsanti preferiti siano aggiornati
    filterProducts();
}
...
function handleRejectCookies() {
    ...
    hideCookieConsent();
    // Se i cookie sono rifiutati, svuota i preferiti e non salvare
    favoriteProductsIds.clear();
    // Non chiamare initAddProducts se i cookie sono rifiutati e la funzionalità dipende da essi
    // Ricarica i prodotti per assicurarsi che i pulsanti preferiti siano aggiornati (non rossi)
    filterProducts();
}
...
function toggleFavorite(productId, buttonElement) {
    if (getCookie("cookie_consent") !== "accepted") {
        ...
        alert("Accetta i cookie per poter aggiungere prodotti ai preferiti.");
...
function calculateProductsPerPage() {
    ...
    // Questo deve corrispondere alle media query in CSS
    let columns;
    if (windowWidth < 300) {
        columns = 1;
    ...
    // Scegli un valore base (ad es. 4 righe) e moltiplicalo per il numero di colonne
    const rows = 4;
    return columns * rows;
}
...
function filterProducts() {
    ...
    // Se la categoria "preferiti" è attiva, mostra solo i preferiti
    if (activeCategories.has('preferiti')) {
...
function createProductCard(product) {
    ...
    productLink.innerHTML += `
        ...
        ${isNewProduct(product.date) ? '<span class="new-badge">Nuovo</span>' : ''}
        <div class="product-info">
            <h3 class="product-title">${product.title}</h3>
            <p class="product-price">${product.price}</p>
            <span class="product-date">Aggiunto il ${formatDate(product.date)}</span>
        </div>
    `;
...
function init() {
    ...
    // Carica i preferiti dai cookie all'avvio
    loadFavoritesFromCookie();
    ...
    // Inizializza le funzionalità di aggiunta prodotti solo se non sono già state inizializzate
    if (getCookie("cookie_consent") === "accepted" && !window.addProductsInitialized) {
        initAddProducts();
        window.addProductsInitialized = true; // Imposta un flag per evitare doppie inizializzazioni
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    init(); // Inizializza il sito (carica i prodotti, ecc.)
    ...
    if (consent !== "accepted" && consent !== "rejected") { // Mostra il modale solo se non c'è una scelta
        ...
        // Se c'è già una scelta, assicurati che lo scrolling non sia bloccato
        document.body.classList.remove('no-scroll');
        // Se i cookie sono stati accettati in una sessione precedente, inizializza addProducts e carica i preferiti
        if (consent === "accepted") {
            loadFavoritesFromCookie(); // Carica i preferiti se il consenso è già dato
            if (!window.addProductsInitialized) {
                initAddProducts();
                window.addProductsInitialized = true;
            }
        }
